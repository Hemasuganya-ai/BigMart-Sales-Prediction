...
'call_llm()' in part4 is a complete, real implementation: it POSTs to a live OpenRouter-style '/chat/completions' endpoint, reads the key from the 'LLM_API_KEY'
environment variable (never hardcoded), and follows every requirement in the task spec (payload shape, headers, status-code check, response parsing).
